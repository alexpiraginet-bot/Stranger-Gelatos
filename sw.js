...
const CACHE = 'stranger-gelatos-2d-v46';
const SPRITES = [
...
self.addEventListener('activate', (e) => {
  // não apaga os caches dos sub-apps (copa- e ninja-), que têm SW próprio
  e.waitUntil(caches.keys().then((keys) => Promise.all(keys.filter((k) => k !== CACHE && !k.startsWith('copa-') && !k.startsWith('ninja-')).map((k) => caches.delete(k)))).then(() => self.clients.claim()));
});
...
self.addEventListener('fetch', (e) => {
  ...
  // Copa e Gelo Ninja têm service worker próprio; a API nunca é cacheada.
  if (url.pathname.startsWith('/copa/') || url.pathname.startsWith('/ninja/') || url.pathname.startsWith('/api/')) return;
  e.respondWith(
